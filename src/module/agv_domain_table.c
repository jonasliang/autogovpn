...
static domain_htable* s_domain_htable = NULL;
 ...
 */
static domain_htable* alloc_domain_htable(size_t htable_size)
{
	int32_t i = 0;
	domain_htable* newdht = NULL;
	domain_node* newhtable = NULL;
	size_t dt_size = sizeof(domain_node) * htable_size;

	newdht = kmalloc(sizeof(domain_htable), GFP_KERNEL);
 ...
 * @return: void
 */
static void free_domain_htable(domain_htable** pdhtable)
{
	int32_t i = 0;
	domain_htable* dhtable = NULL;
	...
	for (i = 0; i < dhtable->m_htable_size; ++i) {
		domain_node* pos = NULL;
		domain_node* next = NULL;
...
int32_t domain_table_load(const domain_list_st* dlist)
{
	...
	for (i = 0; i < dlist->m_size; ++i)	{
		...
		domain_node* newnode = kmalloc(sizeof(domain_node), GFP_KERNEL);
...
{
	int32_t ret = -1;
	domain_htable* tmpdht = rcu_dereference(s_domain_htable);
